...
client.on("interactionCreate",async (interaction, message) => {
  ...
  if(interaction.isButton()) {
     let etkinlikcik = "1062811971336011907"
     let cekiliscik = "1062811972850171924"
     let turnuva = ""
...
client.on("message", async message => {
  ...
  if (komut.YetkiliROL === true && üye.user.username.includes(sunucuData.TAG)) {
      ...
      if (komut.YetkiliROL === true && üye.user.username.includes(sunucuData.TAG)) {
        ...
        if (komut.verilcekRol.some(rol => ["1062801980998815744"].includes(rol))) {
          client.channels.cache.get("1062802658534113410").send(`${message.author} adlı kullanıcı ${üye} adlı kullanıcıya **Vip** rolü verdi! - <@&839884970390847488>`)
        }
