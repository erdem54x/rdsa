...
module.exports = async message => {
    ...
    if (afkReason) {
      ...
      message.channel.send({embeds: [new MessageEmbed().setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${client.emojis.cache.find(x => x.name == "ravgar_tik")} Hey <@${userData.userID}> Başarılı bir şekilde afk modundan çıkış yaptın <t:${Math.floor(Math.floor(userData.Time) / 1000)}:R> afk olmuşsun.`)]}).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
    ...
    message.mentions.members.forEach(async (u) => {
      ...
      let ha = moment(userData.Time).fromNow()
      if (userData.Reason) {
        message.channel.send({embeds: [new MessageEmbed().setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${client.emojis.cache.find(x => x.name == "ravgar_afk")} <@${userData.userID}> isimli üye <t:${Math.floor(Math.floor(userData.Time) / 1000)}:R>, \`${userData.Reason}\` sebebiyle afk modune geçiş yaptı.`)]}).then(e => setTimeout(() => e.delete().catch(() => { }), 10000))
      }
    });
