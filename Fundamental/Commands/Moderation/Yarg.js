...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (banSorumlusu.length >= 1 && client.channels.cache.get(banLogKanal) && banLimit >= 1) {
            ...
            await client.users.fetch(args[0]).then(async user => {
                if (!user) {
                    return message.reply(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir üye belirt ve tekrar dene.`).then(e => setTimeout(() => e.delete().catch(() => { }), 15000)).then(x => x.delete({
                        timeout: 5000
                    }).catch()).catch();
                } else {
                    let reason = args.slice(1).join(" ");
                    if (!reason) return message.channel.send(`${client.emojis.cache.find(x => x.name == "ravgar_cancel")} Geçerli bir sebeb belirt ve tekrar dene.`).then(x => x.delete({
                        timeout: 5000
                    }).catch()).catch();
                    message.guild.bans.fetch().then(async (bans) => {
                        ...
                        .setDescription(`${client.emojis.cache.find(res => res.name === "ravgar_banned")} ${user} (**${user.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle sunucudan **yargılandı.**`)
...
exports.conf = {
    aliases: ["esekoydu", "ravgaruçur", "yargi"]
}
exports.help = {
    name: 'yargı'
}