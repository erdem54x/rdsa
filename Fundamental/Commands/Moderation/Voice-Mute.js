    ...
    if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has("ADMINISTRATOR") || message.author.id === message.guild.owner.id) {
        await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.VMUTEAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_iptal"));
                    return data.VMUTEChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "ravgar_iptal"));
                    return data.VMuteLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "ravgar_tik")))
                }
            })
        ...
        if(!message.member.permissions.has("8") && !server.VMUTEAuthorized.some(rol => message.member.roles.cache.has(rol))  &&!server.UstYetkiliRol.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")}`)
                ...
                Puan: 0
                    ...
                    if (muteSorumlusu.length >= 1 && client.channels.cache.get(muteLogKanal) && muteLimit >= 1) {
                        ...
                        let embedcik = new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({
                            dynamic: true
                        })).setDescription(`${client.emojis.cache.find(res => res.name === "ravgar_vmute")} ${target} (**${target.id}**) kullanıcısı ${message.author} tarafından **${reason}** sebebiyle geçiçi olarak susturuldu. (Ceza Numarası: \`#${cezaID+1}\`)`)
