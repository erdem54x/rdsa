...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.member.permissions.has("ADMINISTRATOR") || durum) {
        ...
        message.guild.fetchVanityData().then(res => {
            message.reply({ embeds: [Embedcik.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setDescription(`**${message.guild.name}** Sunucumuzun Özel-Davet İstatistikleri\n\n\` ➥ \` Sunucu Url [ **${res.code}** ]\n\` ➥ \` Kullanım Sayısı [ **${res.uses}** ]`)] }) }) 
    } else return;
}
exports.conf = {
