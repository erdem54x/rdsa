    ...
    if (message.member.permissions.has("ADMINISTRATOR") || durum) {
        ...
        if (sec === "limit") {
            if (message.member.voice.channel) {
                ...
                message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} Başarılı bir şekilde ses kanalının limitini \`${miktar}\` olarak değiştirdin.`);
            }
        };
        ...
        if (sec === "isim") {
            if (message.member.voice.channel) {
                ...
                if (kanalhak >= 2) return message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_cancel")} Limiti Aştığın için işlem iptal edildi.`);
                client.channels.cache.get(message.member.voice.channel.id).setName(miktar.join(" ")).then(x => {
                    message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} Başarılı bir şekilde ses kanalının adını \`${miktar.join(" ")}\` olarak değiştirdin.`);
                })
            }
        }
