...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (durum) {
        ...
        if (args[0] == "add") {
            ...
            message.reply({ embeds: [Embed.setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_bit")} Sunucuda Rolü Olmayan Kullanıcılara <@&${data.UNREGISTER}> rolü tanılandı. (\`${bg.size}\`)`)] }).catch((err) => console.log(err), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_onay")}`)).then(e => setTimeout(() => e.delete().catch(() => { }), 20000))
        } else if (!args[0]) {
            message.reply({ embeds: [Embed.setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_bit")} Sunucuda Rolsüzlere Verilmek için Tanımlanan Rol;
${client.emojis.cache.find(x => x.name === "ravgar_bit")} Unregister Rol: <@&${data.UNREGISTER}>
${client.emojis.cache.find(x => x.name === "ravgar_bit")} Sunucuda Rolü Bulunmayan Kullanıcı Sayısı \`${bg.size}\`
