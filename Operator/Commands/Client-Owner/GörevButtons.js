...
${client.emojis.cache.find(x => x.name === "ravgar_bit")} Merhaba Görev Seçme Kanalına Hoşgeldin
...
${client.emojis.cache.find(x => x.name === "ravgar_info")} __Seçebileceğiniz Görevler:__

${client.emojis.cache.find(x => x.name === "ravgar_circle")} \` Task I (Ses Görevi) \`Sunucu ses kanallarında saat kasma görevidir. Bu görevde ses kanallarında belirtilen saat ses kasmanız gerekli.

${client.emojis.cache.find(x => x.name === "ravgar_circle")} \` Task II (Mesaj Görevi) \`Mesaj görevi sadece Genel-Chat kanalında atılan mesajlar içindir. Spam,flood veya kurallara aykırı atılan mesajlar için yaptırım uygulanabilir.

${client.emojis.cache.find(x => x.name === "ravgar_circle")} \` Task III (Davet Görevi) \`Sunucumuza üye davet etme görevidir. Davet ettiğiniz üyelerin yan hesap olmaması gerekmektedir.

${client.emojis.cache.find(x => x.name === "ravgar_circle")} \` Task VI (Taglı Çekme Görevi) \`Sunucumuza taglı çekme görevidir. Çektiğiniz taglıların yan hesap olmaması gerekmektedir. Çektiğiniz taglıları "**.taglı @etiket**" veya "**.taglı ID**" komutu ile takımınıza almanız gerekir. Yan hesap tespiti halinde yaptırım uygulanabilir.

${client.emojis.cache.find(x => x.name === "ravgar_circle")} \` Task V (Teyit Görevi) \`Sunucumuzda kayıt yapma görevidir.Sunucumuzdaki kayıtsız üyeleri ses teyit alarak sunucumuza kaydetmelisiniz.
