...
message.channel.send("Kısa sürede 5 den fazla kayıt yaptığınız için yetkileriniz çekildi.")
        ...
        if (!target) return message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`Üye belirtilmedi\` .e/k @ravgar/id Ravgar 20`);
        unreg = unRegisterRol;
        if (!args[1]) return message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`İsim belirtilmedi\` .e/k @ravgar/id Ravgar 20`);

        let name = args[1][0].toUpperCase() + args[1].substring(1);
        let age = Number(args[2]);
        if (!age) message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`Yaş belirtilmedi\` .e/k @ravgar/id Ravgar 20`);
        ...
        if (target.roles.cache.some(rol => erkekRol.includes(rol.id))) return message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`Hatalı İşlem.\` Belirttiğin üye zaten sunucumuzda kayıtlı.`);
        if (target.roles.cache.some(rol => kadinRol.includes(rol.id))) return message.channel.send(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`Hatalı İşlem.\` Belirttiğin üye zaten sunucumuzda kayıtlı.`);
        ...
        let embed2 = new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} ${target} **${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name} | ${age}** Olarak Değiştirildi.`);
        let msg = await message.channel.send({ embeds: [embed2], components: [row] });
        var filter = (button) => button.user.id === message.author.id;
        let collector = await msg.createMessageComponentCollector({ filter, time: 30000 })
        collector.on("collect", async (button) => {
            if(button.customId === "erkek") {
                let embed = new MessageEmbed().setColor("BLACK").setAuthor(message.author.tag, message.author.displayAvatarURL({dynamic: true})).setDescription(`${client.emojis.cache.find(x => x.name === "ravgar_tik")} ${target} kişisinin ismi ${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name} | ${age} olarak değiştirildi \n\n **ERKEK** olarak kayıt edildi.`);
