...
if (interaction.customId === 'two') {
  if (!member.user.username.includes("⚚")) return interaction.reply({ content:"Tagımızı almadıgın için başvuru yapamazsın, tagımız '⚚' tagımızı alıp tekrarden denebilirsin.", ephemeral: true})
  ...
  .addComponents(
    ...
    .setMinLength(3)
    .setMaxLength(20)
    .setPlaceholder('İsim ve yaşını belirt. Örn: Ravgar 20')
    .setRequired(true),
    new TextInputComponent()
    .setCustomId('nekadarzaman')
    .setLabel('Discorda Ayırdığın Süre Miktarı?')
    .setStyle('SHORT')
    .setMinLength(1)
    .setMaxLength(25)
    .setPlaceholder('Örn: Günlük 2 Saat')
    .setRequired(true),
    new TextInputComponent()
    .setCustomId('okuyomu')
    ...
    .setRequired(true),
    new TextInputComponent()
    .setCustomId('nelerkatat')
    .setLabel('Sunucumuza Ne Katabilirsin?')
    .setStyle('LONG')
    .setMinLength(1)
    .setMaxLength(240)
    .setPlaceholder('Örnek: Ses Aktifliğine Faydam Olur.')
    .setRequired(true),
    new TextInputComponent()
    .setCustomId('ytoldumu')
    .setLabel('Önceden Yetkililik Yaptınmı?')
    .setStyle('LONG')
    .setMinLength(1)
    .setMaxLength(240)
    .setPlaceholder('Örnek: Evet Önceden ... Yetkili Oldum')
    .setRequired(true),)
...
if (interaction.values == 'özelcommandhelp') {
...
if (interaction.values == 'yönetimcommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
...
if (interaction.values == 'davetcommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
- .invite (stat [user])
- .topdavet (topdavet)
...
if (interaction.values == 'genelcommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
...
if (interaction.values == 'kurucucommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
...
if (interaction.values == 'yetkilicommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
- .taglı (taglı [user])
...
if (interaction.values == 'moderasyoncommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
- .ban (ban [user] [reason])
- .jail (jail [user])
...
if (interaction.values == 'statcommandhelp') {
  let data = await ozelKomut.find({guildID: member.guild.id}) || [];
interaction.reply({ content:  `\`\`\`
...
\`\`\``, ephemeral: true})
  ...
  client.on('modalSubmit', async (modal) => {
    ...
    if(modal.customId === 'ybasvuru') {
      ...
      const nekadarzaman = modal.getTextInputValue('nekadarzaman'); 
      const okuyormu = modal.getTextInputValue('okuyomu'); 
      const nelerkata = modal.getTextInputValue('nelerkatat'); 
      const öncekiyt = modal.getTextInputValue('ytoldumu'); 
  
      var LogChannel = client.guilds.cache.get("1034868786089709619").channels.cache.find((channel) => channel.id === "1064216071734841384");  
      if (isimyas) {
        const rowcuk = new MessageActionRow().addComponents(
          new MessageButton()
          .setCustomId('yetkiver1nci')
          .setLabel(`1. Yetkiyi ver.`)
          .setStyle('SECONDARY')
          .setDisabled(true) 
,
          new MessageButton()
          .setCustomId('yetkiver2nci')
          .setLabel(`2. Yetkiyi ver.`)
          .setStyle('SECONDARY')
          .setDisabled(true) 
,
          new MessageButton()
          .setCustomId('yetkiver3ncü')
          .setLabel(`3. Yetkiyi ver.`)
          .setStyle('SECONDARY')
          .setDisabled(true) 
,)
  ...
  .setDescription(`
  ${modal.user} (\`${modal.user.tag}\` - \`${modal.user.id}\`) isimli üye bir yetkili başvurusu yaptı.
  Üyenin Formunda doldurdugu mesajlar;
  
  İsim Yaş: \`${isimyas}\`
  Discorda ne kadar süre ayırabilirsin: \`${nekadarzaman}\`
  Okuma/Çalışma durumu: \`${okuyormu}\`
  Sunucumuza neler katabilir: \`${nelerkata}\`
  Sunucumuzda daha önceden yetkili oldu mu: \`${öncekiyt}\`
  
  Aşağıda bulunan butonlar sayesinde üyeye yetki verebilirsin.
  
  Butonlar Bakımdadır \`.yetkili @üye <Yetki Numarası (1,2,3)>\``)
  .setTimestamp()
        await modal.reply({ content: `Yetkili başvurun işleme alındı bu süreçte sabırlı olunuz.`, ephemeral: true });
        await LogChannel.send({  components: [rowcuk],  content: `<@&1064215841308147732> Bir yetkili başvurusu yapıldı!`,  embeds: [embed] })      
      }
    }  
  });
  ...
  if (interaction.customId === 'taskOne') {
    ...
    if (interaction.customId === 'taskTwo') {
      let gorev = await randMiss.findOne({userID: member.id, Active: true }) 
    if(gorev) return interaction.reply({ content: `Zaten aktif bir **${gorev.Mission.MISSION}** görevin var.`, ephemeral: true})
      let MessageChannel  = ["burayachatin id si"];
