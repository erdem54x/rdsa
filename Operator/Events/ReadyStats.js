    ...
    setInterval(async () => {
        ...
        stat.find({guildID: client.ayarlar.sunucuId}, {messageXP: 0, voiceLevel: 0, messageLevel: 0, _id: 0, __v: 0,ondort: 0,total: 0, yirmibir: 0, yirmisekiz: 0, otuzbes: 0, messageCategory: 0, voiceChannel: 0}, async (err, data) => {
            ...
            let sesArr = []
              ...
              sirali2 = sirali2.splice(0, 5).map((user, index) => `<#${user.User}> \`${user.Value} mesaj\``)
    
              let sesMiktar = 0;
              let toplamSesSiralama = "\n"
              data.sort((uye1, uye2) =>
                Number(uye2.totalVoice) - Number(uye1.totalVoice)
              ).splice(0, 20).map((data, index) => {
...
-    client.channels.cache.get("1062802047977672757").messages.fetch("1064241244609445898").then(m => m.edit({embeds: [new MessageEmbed()
  .setFooter(`Developed by ravgar.`).setDescription(`
**${sunucu.name}** Adlı sunucunun en çok ses kasan **Top 20** kullanıcısı aşağıda listelenmiştir!
...
-    client.channels.cache.get("1062802047977672757").messages.fetch("1064241254214418532").then(m => m.edit({embeds: [new MessageEmbed()
  .setFooter(`Developed by ravgar.`).setDescription(`
  **${sunucu.name}** Adlı sunucunun en çok mesaj kasan **Top 20** kullanıcısı aşağıda listelenmiştir!
