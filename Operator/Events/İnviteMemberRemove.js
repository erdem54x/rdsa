...
module.exports = async member => {
 ...
 if (member.user.bot) return channel.send({ content: `**${member.user.tag}** isimli bot, sunucumuzdan Ayrıldı!` })
  ...
  if (!inviteMemberData.Inviter) {
    return channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_cikis")} ${member} İsimli üye **<t:${Math.floor(Math.floor(Date.now()) / 1000)}:R>** \`${member.guild.name}\` Sunucumuzdan Ayrıldı **Bulunamadı** İle Katılmıştı.` });
    ...
    return channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_cikis")} ${member} İsimli üye **<t:${Math.floor(Math.floor(Date.now()) / 1000)}:R>** \`${member.guild.name}\` Sunucumuzdan Ayrıldı **Özel URL** İle Katılmıştı.` });
  } else {
    if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
      ...
      return channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_cikis")} ${member} İsimli üye **<t:${Math.floor(Math.floor(Date.now()) / 1000)}:R>** \`${member.guild.name}\` Sunucumuzdan Ayrıldı ****${inviter.tag}**** Sayesinde Katılmıştı` })
    } else {  
