...
module.exports = async member => {
  ...
  if (member.user.bot && entry) return channel.send({ content: `${member} isimli bot, **${entry.executor.tag}** tarafından ${member.guild.name} Sunucumuza davet edildi.` })
  ...
  if (invite === null) {
    channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_giris")} ${member} İsimli Üye <t:${Math.floor(Math.floor(Date.now()) / 1000)}:R> ${member.guild.name} Sunucumuza Katıldı Fakat Davet Eden *Datada Bulunamadı*.` })
    ...
    return channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_giris")} ${member} İsimli Üye <t:${Math.floor(Math.floor(Date.now()) / 1000)}:R> ${member.guild.name} Sunucumuza **Özel URL** Davetiyle Katıldı.` })
    ...
    if (Date.now()-member.user.createdTimestamp <= 1000*60*60*24*7) {
      ...
      channel.send({ content: `${client.emojis.cache.find(x => x.name === "ravgar_giris")} ${member} İsimli Üye <t:${Math.floor(Math.floor(Date.now()) / 1000)}:R> ${member.guild.name} Sunucumuza **${invite.inviter.tag}** Davetiyle Katıldı.` });
